...
public interface HitRepository extends JpaRepository<Hit, Long>, JpaSpecificationExecutor<Hit> {
    ...
    @Query("select  count (hitId) " +
            "from hits " +
            "where eventId = ?1")
    long getCountHitsByEventId(Long id);

    @Query("select  new ru.practicum.ewm.client.dto.UtilDto(h.eventId, count (h)) " +
            "from hits as h " +
            "where h.eventId in ?1 " +
            "group by h.eventId")
    ...
    List<HitResponseDto> findViewStats(@Param("uris") List<String> uris,
    ...
    @Query("SELECT new ru.practicum.ewm.client.dto.HitResponseDto(h.uri, COUNT(DISTINCT h.ip)) " +
            ...
            "GROUP BY h.app, h.uri " +
            "ORDER BY COUNT(h.ip) DESC")
    List<HitResponseDto> findViewStatsUniqueIp(@Param("uris") List<String> uris,
                                               @Param("start") LocalDateTime start,
                                               @Param("end") LocalDateTime end);
