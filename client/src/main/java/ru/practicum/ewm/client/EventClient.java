...
//    public static final String BASE_PATH = "http://localhost:9090";
    ...
    public EventClient(@Value("http://localhost/8080") String serverUrl, RestTemplateBuilder builder) {
        ...
        viewsRest = builder
                .uriTemplateHandler(new DefaultUriBuilderFactory(serverUrl + "/views"))
                .requestFactory(HttpComponentsClientHttpRequestFactory::new)
                .build();
    }
    ...
    public void postHit(String endpoint, String clientIp, Long eventId) {
        ...
        hitRest.exchange(BASE_PATH + "/hit", HttpMethod.POST, requestEntity, HitPostDto.class);
    }

    public ResponseEntity<Long> getViewsByEventId(Long eventId) {
        return viewsRest.exchange(BASE_PATH + "/views" + "/" + eventId, HttpMethod.GET, null, Long.class);
    }

    public List<UtilDto> getViewsByEventIds(List<Long> eventIds) {
        StringBuilder pathBuilder = new StringBuilder();
        pathBuilder.append(BASE_PATH).append("/views").append("?ids=");
        Map<String, Object> params = new HashMap<>();

        for (int i = 0; i < eventIds.size(); i++) {
            pathBuilder.append(eventIds.get(i)).append(",");
            String key = "id" + i;
